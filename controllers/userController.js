...
import { Address, Token, User } from "../models/userModel.js";
...
export const register = catchAsyncError(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "TechyDeals Account Verification",
      message: `Your account verification token is :- \n\n ${url} \n\n If you have not requested this email then, please ignore it.`,
    });
...
export const login = catchAsyncError(async (req, res, next) => {
  ...
  if (!user.verified) {
    ...
    return next(
      new ErrorHandler("Verification link has been sent to your email.", 401)
    );
  }
...
//User Verified
export const userVerified = catchAsyncError(async (req, res, next) => {
...
export const forgotPassword = catchAsyncError(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "TechyDeals Password Recovery",
      message,
    });
...
export const updateAddress = catchAsyncError(async (req, res, next) => {
  ...
  if (!name || !email || !phone || !city || !street || !state || !zip) {
    return next(
...
export const contactUs = catchAsyncError(async (req, res, next) => {
  ...
  sendEmail({
    ...
    subject: "Techydeals Contact Us",
    message: `<center>Site: TechyDeals</center> \n Name: ${name} \n Email: ${email} \n Message: ${message}`,
  });
